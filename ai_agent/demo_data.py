...
# Canned Athena rows keyed by a substring that appears in the corresponding
# tool's SQL template. Covers every multi-step sample question shown in the
# Command Center plus the pipeline-health check.
_CANNED_ATHENA: dict[str, list[dict]] = {
    ...
    "user_churn_prediction": [
        ...
        # by='plan' query is matched separately via the "GROUP BY plan_id" key.
    ],
    "GROUP BY plan_id": [
    ...
    if "SELECT COUNT(*)" in upper:
        for table, count in _CANNED_COUNTS.items():
            if table in sql:
                return [{"cnt": count}]
        return [{"cnt": "0"}]
    for key, rows in _CANNED_ATHENA.items():
        if key in sql:
            return rows
    return []
